...
int main(int argc, char *argv[]) {
  ...
  if(world_rank == 0){
       ...
       printf("%d, %d, %d, %f", world_rank, startx, endx, time_taken);

       printf("Temp[%d,%d]=%f",0,0,T->temp(0,0));
       for(int i= 1; i <5; i++){
         int index = i*(grid_size/5);
         if(index <= endx && index >=startx){
           printf("Temp[%d,%d]=%f",index,index,T->temp(index,index));
         }
       }
       printf("Temp[%d,%d]=%f",endx,endx,T->temp(endx,endx));
  ...
  if(world_rank == 0){
    ...
    printf("Time taken (in seconds) : %f", global_time);
  }
  return 0;
}
